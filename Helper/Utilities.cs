...
namespace Molla.Utilities {
    public static class Utilities {
        ...
        public static async Task<string?> UploadFile(IFormFile file, string? newname){
            try
            {
                if(newname == null) newname = file.FileName;
                Console.WriteLine(file.FileName);
                var filePath = Path.Combine(Directory.GetCurrentDirectory(),"wwwroot/uploads", newname);
                var supprtType = new []{"jpg", "jpeg", "png", "gif"};
                var fileExt = Path.GetExtension(file.FileName).Substring(1);
                if(!supprtType.Contains(fileExt)){
                    return null;
                }
                else{
